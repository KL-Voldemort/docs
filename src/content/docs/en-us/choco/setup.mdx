...
# If the policy is Restricted, set it to Bypass (temporarily)
if ($currentPolicy -eq "Restricted") {
    Set-ExecutionPolicy Bypass -Scope CurrentUser -Force
}

# Download and execute the Chocolatey installation script securely
Invoke-Expression (Invoke-WebRequest -Uri 'https://community.chocolatey.org/install.ps1' -UseBasicParsing).Content

# Restore the original execution policy if it was changed
if ($currentPolicy -eq "Restricted") {
    Set-ExecutionPolicy $currentPolicy -Scope CurrentUser -Force
}

# Add Chocolatey to the PATH (optional)
if (!(Test-Path -Path "%ALLUSERSPROFILE%\chocolatey\bin")) {
    Write-Warning "Chocolatey installation directory not found. Skipping PATH update."
} else {
    $path = "$env:PATH;" + "%ALLUSERSPROFILE%\chocolatey\bin"
    $env:PATH = $path
}
